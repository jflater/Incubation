...
```
```{r Loading libraries, messages = F, echo=FALSE}
library(phyloseq)
library(vegan)
library(tidyverse)
library(viridis)
...
```{r Make phyloseq, messages = F, echo=FALSE}
...
rm(inc.inputs)
```
```{r rarefy and ordinate, messages = F}
...
kable(input.PCoA.stats$aov.tab, align = "c", format = "html", digits = 3)
```
