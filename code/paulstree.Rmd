...
p <- ggtree(resp_p$data, aes(color = Phylum.x))
p
```
```{r, message=F, error=F, warning=F}
colnames(df2)
...
df4 <- df2 %>%
  select(label, Phylum, Class, Order, Family, Genus, Alfalfa_Early = Alfalfa_early_log2FoldChange, Alfalfa_Late = Alfalfa_late_log2FoldChange, Compost_Early = Compost_early_log2FoldChange, Compost_Late = Compost_late_log2FoldChange, Mix_Early = Mix_early_log2FoldChange, Mix_Late = Mix_late_log2FoldChange) %>%
  column_to_rownames(var = "label")

write.csv(df3, file ="../data/resp_table.csv")
write.csv(df4, file ="../data/Table_5.csv")
final_resp <- test %>% 
  gheatmap(df3, offset= 2, width= .8, font.size = 2.5, colnames=T, colnames_angle=-45, hjust=0, color = "white", legend_title = "Log2Fold Change")
final_resp
```
```{r, message=F, error=F, warning=F}
...
tree_plot_data
p <- tree_plot  + 
  geom_hilight(node = 232, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 201, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 212, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 183, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 186, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 193, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 158, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 132, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 124, fill = "gray", alpha=0.4, extend = 3) + 
  geom_hilight(node = 126, fill = "gray", alpha=0.4, extend = 3) 
p
pp <- p %<+% new.labels +
  geom_nodelab(aes(label=new.label), size=4, color="black", alpha = 1) + 
  scale_fill_viridis_c() +
  guides(fill=guide_colorbar(title="Log2Fold Change"))
pp 
  
ggsave("../Figures/resp_tree_wnodes.png", plot = pp, device = "png", width = 18.5, height = 32, units = "cm", dpi = 350)
```
