...
final_resp <- test %>% 
  gheatmap(df3, offset= 2, width= .8, font.size = 2.5, colnames=T, colnames_angle=-45, hjust=0, color = "white") +
  ggtitle("Log fold change of genera for each treatment response group. \nMin LFC = 4 when comparing to reference")
final_resp
```
```{r, message=F, error=F, warning=F}

tree_plot <- final_resp + scale_fill_viridis(option = "viridis") +
  guides(color=guide_legend(title="Log fold change"))
tree_plot
