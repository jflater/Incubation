...
resp_p <- ggtree(inc_resp, branch.length = "none") %<+% df2 +
  geom_tiplab(aes(label=Phylum), size=1, align=TRUE, linesize=.1) +
  theme_tree() 
...
```{r, message=F, error=F, warning=F, echo=F}
final_resp <- resp_p  %>% 
  gheatmap(df3, offset=1, width=0.1, font.size = 1, colnames=T, colnames_angle=-45, hjust=0, color = "black") +
