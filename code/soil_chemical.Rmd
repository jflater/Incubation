...
day_ggboxplot <- function(df){
  ggboxplot(data = df,x = "treatment"
    ...
    stat_compare_means(aes(label = ..p.signif..), method = "t.test", ref.group = "Reference", paired = TRUE, size = 2)# +
   # stat_compare_means(method = "anova", label.y = max(data$MBC_mg.kg_per_dry_wt_soil), size = 2, position = position_nudge(x = 1, y = 3))
}
p <- day_ggboxplot(data) +
  facet_grid(~day) +
  theme(legend.position = "none")
p
ggsave("../Figures/Fig_S4.png", plot = p + theme_my() +
  theme(legend.position = "none"), device = "png", width = 180, height = 90, units = "mm", dpi = 500)
```
