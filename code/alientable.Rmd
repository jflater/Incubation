...
physeq <- readRDS("../data/physeq2020.RDS")
```
```{r, warning=F, echo=F}
alf <- subset_samples(physeq, treatment %in% c("Alfalfa") & day %in% c("7", "14", "21", "35", "49", "97")) %>%
...
```
```{r, warning=F, echo=F}
comp <- subset_samples(physeq, treatment %in% c("Compost") & day %in% c("7", "14", "21", "35", "49", "97")) %>%
  filter_taxa(function(x) sum(x) > 1, T)
...
```
```{r, warning=F, echo=F}
mix <- subset_samples(physeq, treatment %in% c("Mix") & day %in% c("7", "14", "21", "35", "49", "97")) %>%
  filter_taxa(function(x) sum(x) > 1, T)
